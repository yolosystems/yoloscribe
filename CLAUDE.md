...
Provider / model / credential resolution lives in the **LiteLLM config**, not in YoloScribe code. The model keys used by `YOLOSCRIBE_*_MODEL` env vars and the `model:` frontmatter field (`haiku`, `sonnet`, `opus`, `glm-flash`, `glm-5`, `text-embedding-3-small`, `bedrock-*`) are the `model_name` entries in the deployed proxy's config — `yoloscribe-ops/helm/litellm.<stage>.<region>.values.yaml` under `proxy_config.model_list`, mirrored for local dev in `infra/litellm/config.example.yaml` — edit that file (or your deployed LiteLLM config) to add/change models, credentials, or providers. `LITELLM_BASE_URL` must be set; there is no per-provider fallback. Empty keys resolve to `DEFAULT_MODEL_KEY` (`sonnet`).
